...
public class DownloadView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, // 컨트롤러에서 전달한 데이터 수신 
		...
		FreeBoardAttachDto freeBoardAttach = (FreeBoardAttachDto)model.get("attach");  // controller 에서 전달한 데이터 읽기
		...
		resp.addHeader("Content-Disposition", 
				"Attachment;filename=\"" + 
				new String(freeBoardAttach.getAttachFileName().getBytes("utf-8"), "ISO-8859-1") + "\"");
		
		//ServletContext : JSP의 application객체와 동일한 객체
		ServletContext application = req.getServletContext();
		
		String path = application.getRealPath("/resources/upload/" + freeBoardAttach.getSavedFileName());
